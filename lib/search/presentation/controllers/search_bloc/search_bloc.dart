...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  SearchBloc(this._searchUseCase) : super(const SearchState()) {
    on<GetSearchResultEvent>(_getSearchResults, transformer: debounce(_duration));
  }
  ...
  Future<void> _getSearchResults(
    ...
    result.fold(
        (l) => emit(state.copyWith(
            status: SearchRequestStatus.error, message: l.message)), (r) {
      if (r.isEmpty) {
        emit(
          state.copyWith(status: SearchRequestStatus.noResults),
        );
      } else {
        emit(
          state.copyWith(status: SearchRequestStatus.loaded, searchResults: r),
        );
      }
    });
  }
